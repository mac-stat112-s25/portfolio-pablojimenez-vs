...
There doesn't seem to be a lot of relationship between them.
...
We would like to see the responses of each individual
b.  What about a visualization of the *quantitative* `elevation` variable?
We would also like to see them portrayed on a graph that understand their patters
...
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```
# Board appeared to represent the distribution of data based on the responses, this is because we added bars with (geom_bar())
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```
# We added and defined the w axis in the plot
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
# We set the color of the bars to blue
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  ...
  labs(x = "Rating", y = "Number of hikes")
```
# We set the lines of the graphs to orange and the fill is blue 
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  ...
  theme_minimal()
```
# it changed the background (but I can imagine that these some of background font)
...
It's used to modify more details of the same plot 
-   We added the bars using `geom_bar()`? Why "geom"?
I would say that it is because it sets the geometrical form
-   What does `labs()` stand for?
They represent the axis of the graph
-   What's the difference between `color` and `fill`?
Color is the lines of the from chosen and fill is the color that the overall form would have
...
We see: difficult, easy and moderate
-   **variability between categories**: Are observations evenly spread out among the categories, or are some categories more common than others?
There are more observation in moderate, then easy and lastly in difficult

We must then *translate* this information into the *context* of our analysis, here hikes in the Adirondacks. Summarize below what you learned from the bar chart, in context.

We learned that most of the hikers categorized the Adirondacks hike as a moderate hike, about 10 people rated it as easy, and 8 people categorized it as difficult
...
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
# Tehre are white spaces between the histograms
ggplot(hikes, aes(x = elevation)) +
