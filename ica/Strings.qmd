...
# Yet another way
# classes |> 
#  filter(str_sub(instructor))
```

I think I still need to figure out how does str_sub entirely works
### Example 4
...
# Which one is the department with the highest total enrollment

```{r}
enrollments |>
  group_by(dept) |>
  summarize(total_enrollment = sum(enroll, na.rm = TRUE),
            n_sections = n()) |>
...
enrollments |>
  ...
  summarize(total_enrollment = sum(enroll, na.rm = TRUE)) |>
  arrange(desc(total_enrollment)) |>
  head(10) |> # Focus on the top 10 for visualization
  ggplot(aes(x = reorder(dept, total_enrollment), y = total_enrollment)) +
  geom_col() +
  labs(title = "Total Enrollment by Department (Top 10)",
       x = "Department",
       y = "Total Enrollment")
```
...
# WHich insructor is the one that have the highest toal enrollment across their sections?
