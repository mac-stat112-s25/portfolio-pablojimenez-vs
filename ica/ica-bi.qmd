...
```{r}
#I am wondering how could I implement other categories in the plot
```
...
There is a strong positive relationship and the states that deveiate from the national trend are DC and TX
...
# It wouldn't be an effective visualization since the data variate from each group and doesn't give space to make an effective correlation
...
The red county gave more support to the Republicans and then the purple and blue were the next ones to give more support
...
#- It doesn't give more information other than very general axis
#- I doesn,t highlight its units
...
It fills the graphs with different colors 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
# It sets the opacity of the density graphs
...
# It separates the different graphs on their own table
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# because it mixes different sets of data
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
I like histograms and correlations boxplots
-   What is one pro of density plots relative to boxplots?
Thaat you can different sets of data by themselves and examine their own variables

-   What is one con of density plots relative to boxplots?
That they cannot analyze the spread of data in the same way generally as boxplots do
...
You can identify different groups within one set of data but the bad thing is that you can't see beyond to their position on the spectrum of answers

#### Part b {.unnumbered}

What's your favorite bar plot from part and why?

I like that histograms because they are very understandable
