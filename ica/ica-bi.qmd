...
# THe plots are on top of each other
...
```{r eval = FALSE}
# What does scale_fill_manual do? it defines colors to fill categories
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r eval = FALSE}
# What does alpha = 0.5 do? adds transparency
...
# separates the density plots into "facets" for each historical group
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
Doesn’t oversimplify the data / boil the data down to just 5 numbers.

-   What is one con of density plots relative to boxplots?

Boxplots can be easier to interpret
...
One pro is that is easier to compare the relative outcomes in blue vs purple vs red states 

One con is that you lose track of how many counties fall into blue vs purple vs red states

#### Part b {.unnumbered}

What's your favorite bar plot from part and why?

I think a facet bar is quite an accurate way of representing data while giving it the emphasis that it deserves.
