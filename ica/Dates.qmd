...
total_births_by_state <- Birthdays %>%
  group_by(state) %>%
...
daily_births <- Birthdays %>%
  group_by(date, wday) %>%
  summarise(total_births_on_day = sum(births))
```

Using this data, construct a plot of `births` over time, indicating the day of week.

```{r}
ggplot(daily_births, aes(x = date, y = total_births_on_day, color = wday)) +
  geom_line() +
  labs(x = "Date",
       y = "Total Births",
       color = "Day of Week") +
...
ggplot(monthly_births, aes(x = factor(month_name, levels = month.abb), y = total_births, group = year, color = factor(year))) +
  geom_line() +
  geom_point() +
  labs(x = "Month",
       y = "Total Births",
...
births_1988_by_week_state <- Birthdays %>%
  filter(year == 1988) %>%
  mutate(week = strftime(date, format = "%W")) %>%
  filter(week != "53") %>%
  group_by(state, week) %>%
  summarise(total_births = sum(births))

print(births_1988_by_week_state)

ggplot(births_1988_by_week_state, aes(x = as.integer(week), y = total_births, color = state)) +
  geom_line() +
...
births_1988_by_week_MN_LA <- Birthdays %>%
...
print(births_1988_by_week_MN_LA)

ggplot(births_1988_by_week_MN_LA, aes(x = as.integer(week), y = total_births, color = state)) +
  geom_line() +
  labs(x = "Week of the Year",
       y = "Total Births",
       color = "State") +
...
massachusetts_births_1979_sorted <- Birthdays %>%
  filter(state == "MA", year == 1979) %>%
  arrange(desc(births))

print(head(massachusetts_births_1979_sorted))
...
top_birth_states_sept_1979 <- Birthdays %>%
...
print(top_birth_states_sept_1979)
```
