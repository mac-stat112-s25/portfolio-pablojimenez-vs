...
The boxplot shows the distribution of energy usage through months per each address. Address 'b' generally shows more energy usage espcially in colder months.

```{r}
ggplot(energy, aes(x = factor(month), y = therms, fill = address)) +
  geom_boxplot() +
  labs(title = "Energy Use (Therms) by Month and Address",
       x = "Month",
       y = "Therms",
       fill = "Address")
```
...
The plot shows the trend of energy usage through time in both addresses. We can see seasonal fluctuations, with higher use during winter months, and address 'b' is the one that uses more energy.

```{r}
ggplot(energy, aes(x = date, y = therms, color = address)) +
  geom_line() +
  labs(title = "Energy Use Over Time by Address",
       x = "Date",
       y = "Therms",
       color = "Address")
```
...
The plot says that energy has increased after renovation

```{r}
ggplot(energy, aes(x = renovated, y = therms, fill = address)) +
  geom_boxplot() +
  labs(title = "Energy Use Before and After Renovation",
       x = "Renovated",
       y = "Therms",
       fill = "Address")
```
...
Renovations happened during warmer months, when energy usage is naturally lower. 

```{r}
ggplot(energy, aes(x = hdd, y = therms, color = renovated)) +
  geom_point() +
  facet_wrap(~ address) +
  labs(title = "Energy Use vs. Heating Degree Days",
...
Starbucks is mainly in first world countries 
