...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Desktop/CS 2024/GitHub/portfolio-pablojimenez-vs/data/imdb_5000_messy.csv")
```
...
imdb_messy |>
  summarize(n_actor_1_facebook_likes = sum(is.na(actor_1_facebook_likes)))
```
...
There is no need to entirely remove the related movies from the dataset
...
-   What did `drop_na()` do? How many data points are left? It removed any movie with any NAs
-   In what situations might this function be a good idea? when we need to complete info on every variable 
-   In what situations might this function be a bad idea? When there would be only some variables neede to be completed.
